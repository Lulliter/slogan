...
## 1) Select and engineer the features [using `projs_train2`]
...
#   rename(., proj_id = "id") %>%
   # recode as factors 
   mutate (across (c(status, FY_appr, regionname, countryname, sector1, 
...
# proportion_missing <- projs_train2 %>%
...
## [👍🏻] Logistic with `pdo` + `sector_f`|`regionname`|`FY_appr` as predictors
...
## [🫤] Naive bayes classific with text and `sector_f` and `regionname` and `FY_appr` as predictors
