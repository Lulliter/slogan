...
I also created a *"broad sector" variable* to group the sectors in broader definitions:
...
# Evaluate the title with glue first
title_text <- glue::glue("Sector words frequency in PDO over Fiscal Years between FY {min(pdo_train_t$boardapprovalFY)} and {max(pdo_train_t$boardapprovalFY)}") 
# Plot
...
proj_sect_broad_fr <-  ggplot(data = sector_broad, 
   ...
   facet_wrap(~tok_sector_broad, ncol = 2, scales = "free")+   guides(color = FALSE) +
   theme_bw()+
...
#### --- I could see if corresponds to sector flags

#### --- I could see if corresponds to any WDR publications

## Institution-related BIGRAM frequency

### Isolate INSTITUTIONs words and see frequency over years
...
So, I will try to find consecutive bigrams "public sector" and "private sector" to get an indicative frequency of these terms. 
...
```{r}
# Create bigrams by pairing consecutive tokens by sentence ID and their token IDs
bigrams <- pdo_train_t %>%
...
#### [FIG] Bigrams ("public sector", "private sector") freq ggplot
```{r}
...
# Evaluate the title with glue first
title_text <- glue::glue("Frequency of bigrams \"public sector\" and \"private sector\" in PDOs over Fiscal Years between FY {min(sector_bigr_df$FY_appr)} and {max(sector_bigr_df$FY_appr)}") 
