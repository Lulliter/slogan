...
## Illustrative PDOs text in Projects' documents
```{r}
#| eval: true
#| echo: false
#| output: true
...
Here I will just load the pre-processed data.
...
pdo_t <- pdo_train_t %>% 
   select(proj_id, pdo,pr_name, FY_appr, FY_clos, status, regionname, countryname,sector1) %>% 
   group_by(proj_id) %>% 
   slice(1)
