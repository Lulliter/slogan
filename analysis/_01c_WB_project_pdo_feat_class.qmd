...
# 1 accuracy    binary     0.754    10 0.0118  Preprocessor1_Model1
# 2 brier_class binary     0.170   10 0.00503 Preprocessor1_Model1
# 3 roc_auc     binary     0.785    10 0.0101  Preprocessor1_Model1

# OLD 
collect_metrics(env_lasso_cv)
#1 accuracy    binary     0.745     10 0.00997 Preprocessor1_Model1
#2 brier_class binary     0.174    10 0.00371 Preprocessor1_Model1
#3 roc_auc     binary     0.776    10 0.00813 Preprocessor1_Model1
```

Improved a little `accuracy` but with a slight decrease in `roc_auc`  and `brier_class` compared to the previous model!
...
# ---  Create a recipe with a token filter step that excludes stopwords
# Rebuild recipe with tokenfilter step
env_FEAT_recipe <- recipe (env_cat_f2 ~ pdo + sector1 + regionname + boardapprovalFY,
   ...
   step_unknown(sector1 ,new_level = "Unknown sect" ) %>%
   step_unknown(regionname ,new_level = "Unknown reg" ) %>%
   step_unknown(boardapprovalFY ,new_level = "Unknown FY" ) %>%
   # convert to dummy variables
   step_dummy(sector1, regionname, boardapprovalFY, one_hot = TRUE) 
```
check what changed... 
```{r}
# prep and bake the recipe
...
env_FEAT_fitted_coeff <- env_FEAT_fit %>% 
   ...
   arrange(-estimate)
```
+ "sector1... " appear among the top coefficients!!!
...
var_importance_tbl %>%
   ...
   geom_point() +
   coord_flip() +
   facet_wrap(~EnvRiskOutcome, ncol = 1, scales = "free_y") +
   labs(
      title = glue::glue ("Most influential features for predicting Environmental Risk Cat., ranked by *Importance*"),
       subtitle = "Regularized Logistic Regression model on text + metadata tags",  # Add subtitle explaining the y-axis
...
## [🫤] Naive bayes classific with text and `sector1` and `regionname` and `boardapprovalFY` as predictors
...
```
Worse than logistic!
<!-- 1 accuracy    binary     0.677    10  0.0101 Preprocessor1_Model1 -->
<!-- 2 brier_class binary     0.323    10  0.0100 Preprocessor1_Model1 -->
<!-- 3 roc_auc     binary     0.630    10  0.0198 Preprocessor1_Model1 -->
...
-   Evidently, this project was primarily a learning / proof-of-concept exercise, not too concerned with the evaluating ML models' performance, nor with in-depth analysis of the data.

-   Nevertheless, even this initial exploration demonstrated the potential of applying NLP techniques to unstructured text data, such as World Bank project documents, to achieve various goals, including:

    -   detecting frequency trends of sector-specific language, and ideas over time,
    ...
    -   trigger additional questions that could lead to further research.
...
-   It is quite clear that the above techniques for exploration and topic modeling are most effective when paired with domain expertise that can inform how to interrogate the data and what to look for.

-  While AI and large language models (LLMs) are currently in the spotlight, this kind of analysis might seem like yesterday's news. However, I believe there is still a huge untapped potential for meaningful applications of NLP and text analytics in development studies, policy analysis, and other areas.

-  A pain point in this type of work is efficiently retrieving input data from document corpora. Despite the World Bank's "Access to Information" policy, programmatic access to its extensive text data resources is still quite lacking (no dedicated API, various stale pages and broken links). This should be addressed, perhaps following the model of the World Development Indicators (WDI) data, much more accessible and well-curated.
...
-  [Guide from Penn Libraries](https://guides.library.upenn.edu/penntdm/r)
