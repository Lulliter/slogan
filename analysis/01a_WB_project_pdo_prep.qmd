...
## 🟡 themes 
```{r}
all_proj_themes_25 <- readxl::read_excel(here::here (
...
all_proj_theme_1_25 <- all_proj_themes_25 %>% 
...
```{r}
all_proj_sectors_25 <- readxl::read_excel(here::here (
   "data", "raw_data", "project3","all_projects_as_of31mar2025.xlsx"), 
...
all_proj_sector_1_25 <- all_proj_sectors_25 %>% 
...
# save next to the original file
save(all_proj_geo_25, file = here::here("data", "raw_data", "project2", 
...
```{r}
all_proj_financiers_25 <- readxl::read_excel(here::here (
   "data", "raw_data", "project3","all_projects_as_of31mar2025.xlsx"), 
...
skimr::skim(all_proj_t$pdo) # complete_rate 0.707
```
...
## 🟢 Join sector1 and theme1 to `all_proj_t`

```{r}
all_proj_t <- left_join(all_proj_t, all_proj_theme_1_25, by = c("id" = "project_id")) %>% 
...
f_recap_values(all_proj_t, c("id","pdo", "projectstatusdisplay", 
...
chisq_test2
# data:  observed
# X-squared = Inf, df = 18, p-value < 0.00000000000000022
...
There are `r nrow(duplicates)` projects with NON-UNIQUE PDO text in the dataset. In some cases, evidently, the same PDO is used for multiple projects (from a minimum of 2 to a maximum of 9 time!!!), most likely when there is a *parent* project or subsequent phases of the same.
