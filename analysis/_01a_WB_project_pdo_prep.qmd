...
+ **9,773** (non missing) PDOs
+ 22,569-4981 = **17,588** (non missing) boardapprovalFY 
...
f_count_missing_values <- function(data, columns) {
...
f_count_missing_values(all_proj_t, c("id","pdo", "projectstatusdisplay", "boardapprovalFY", "sector1", "theme1"))
```

#### Which PDOs are missing? 
AT least I hope the y are missing in a non systematic way so I comare the distributions across some key features of **ALL PROJs** v **NON-missing-PDO-PROJs**
...
Based on some available proj features, I would say that even if many projects have missing feature value, PDO texts' missingness seems to happen at random, except maybe for:
...
## ---- Annotate [LOOONG] with `cleanNLP::cnlp_annotate()`
```{r}
...
pdo_train_tag_t <- pdo_train_tag_t %>% 
   mutate(word_original = token) %>%
   
   # Replace placeholders in the 'token' column
   mutate(token = case_when(
      str_detect(word_original, "HYPHENWORD") ~ str_replace_all(word_original, "HYPHENWORD", "-"),
      str_detect(word_original, "SCALEUPPLACEHOLDER") ~ str_replace_all(word_original, "SCALEUPPLACEHOLDER", "scale-up"),
      str_detect(word_original, "FOLLOWUPPLACEHOLDER") ~ str_replace_all(word_original, "FOLLOWUPPLACEHOLDER", "follow-up"),
      str_detect(word_original, "KNOWHOWPLACEHOLDER") ~ str_replace_all(word_original, "KNOWHOWPLACEHOLDER", "know-how"),
      str_detect(word_original, "COVIDPLACEHOLDER") ~ str_replace_all(word_original, "COVIDPLACEHOLDER", "covid-19"),
      TRUE ~ word_original
   )) %>%
   ...
   relocate(word_original, .after = doc_id) # 225,360
```
