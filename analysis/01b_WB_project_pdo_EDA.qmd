...
my_pretty_theme <- theme_minimal(base_family = "roboto_condensed", base_size = 12) +
  theme(panel.grid.minor = element_blank(),
        ...
        plot.subtitle = element_text(face = "plain", size = rel(1.3), color = "#A6A6A6"),
...
+ **WAT_SAN** = water, wastewater, sanitation
...
pdo_train_t <- pdo_train_t %>%
   ...
   mutate(tok_sector_broad = case_when(
      str_detect(token_l, regex("water|wastewater|sanitat|Sewer|Irrigat|Drainag", ignore_case = T)) ~ "WAT_SAN",
      str_detect(token_l, regex("transport|railway|road|airport|waterway|bus|metropolitan|inter-urban|aviation", ignore_case = T)) ~ "TRANSPORT",
      token_l == "port" ~ "TRANSPORT",
      str_detect(token_l, regex("urban|housing|inter-urban|peri-urban|waste", ignore_case = T)) ~ "URBAN",
      str_detect(token_l, regex("energ|electri|hydroele|hydropow|renewable|transmis", ignore_case = T)) ~ "ENERGY",   
      str_detect(token_l, regex("health|hospital|medicine|drugs|epidem|pandem|covid-19|vaccin", ignore_case = T))  ~ "HEALTH",
      str_detect(token_l, regex("educat|school|vocat|teach|univers|student|literacy|training|curricul", ignore_case = T)) ~ "EDUCATION",
      # not infra 
      str_detect(token_l, regex("Agricultural|Agro|Fish|Forest|Crop|lovestock", ignore_case = T)) ~ "AGR FOR FISH",
      str_detect(token_l, regex("Minin|oil|gas|mineral", ignore_case = T)) ~ "MINING OIL&GAS",
      str_detect(token_l, regex("Social Protec", ignore_case = T)) ~ "SOCIAL PROTECTION",
      str_detect(token_l, regex("Bank|finan|Investment", ignore_case = T)) ~ "FINANCIAL",
      str_detect(token_l, regex("Information|Communication|ICT|Internet", ignore_case = T)) ~ "ICT",
      str_detect(token_l, regex("Tourism|Trade and Services|Manuf", ignore_case = T)) ~ "IND TRADE SERV",
      str_detect(token_l, regex("Government|Public Admin|Institution|Central Agenc|Sub-national Gov|law|justice|governance", ignore_case = T)) ~ "INSTITUTIONAL SUPP.",
      TRUE ~ NA_character_)) %>% 
...
sector_broad <- pdo_train_t %>% 
   ...
   filter(tok_sector_broad %in% c("WAT_SAN", "TRANSPORT", "ENERGY","MINING OIL&GAS", "URBAN","ICT", "HEALTH", "EDUCATION")) %>%
   count(FY_appr, tok_sector_broad) %>% 
   filter(n > 0) %>% 
   mutate(tok_sector_broad = factor(tok_sector_broad, levels = c("WAT_SAN", "ENERGY", "TRANSPORT",
                                                                 "URBAN","MINING OIL&GAS","ICT", 
                                                                 "HEALTH", "EDUCATION"))) # reorder values by frequency
#df$FY
...
#### [FIG fun] split sector (`tok_sector_broad`) freq ggplot
```{r}
