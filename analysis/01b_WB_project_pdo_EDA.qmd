...
projs_train <- projs_train_bef2001  |> 
   ...
   dplyr::filter(.data$boardapprovalFY > 2001) |>    # 4425
   dplyr::rename(
      # new = old
      proj_id = id,
...
pdo_train_t <- readRDS(here::here("data", "derived_data", "pdo_train_t.rds")) %>%           # 314 k 
   # 2001 was to be shure there are pdo  (2024 is the last year with complete data)
   dplyr::filter(FY_appr > 2001)  #|>           # 252 k  
   # dplyr::rename(
   #    # new = old
   #    #proj_id = id,
   #    #status = projectstatusdisplay ,
...
Lastly (and this was quite surprising to me) the **remaining, viable `r sum(tracking[14,2])` unique projects**, were matched by **only 9,914 unique PDOs**!
In fact, **2,461 projects share 1,097 NON-UNIQUE PDO text** in the "cleaned" dataset. Why? Apparently, the same PDO is re-used for multiple projects (from 2 to as many as 13 times), likely in cases of follow-up phases of a *parent* project or components of the same lending program."
...
We are looking at (**training data subset**) `pdo_train_t` which has `r nrow(pdo_train_t)` rows and `r ncol(pdo_train_t)` columns obtained from `r nrow(projs_train)` PDOs (of which `r n_distinct(pdo_train_t$pdo)` unique) of `r n_distinct(pdo_train_t$proj_id)` Wold Bank projects approved in Fiscal Years ranging from `r min(pdo_train_t$FY_appr)` to `r max(pdo_train_t$FY_appr)`. 
...
train_recap <- tibble(
  ...
  counts = c(
    n_distinct(pdo_train_t$proj_id),
