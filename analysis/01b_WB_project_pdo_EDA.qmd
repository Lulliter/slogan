...
# TEXT ANALYTICS -------------------------------------
library(tidytext) # Text  MINING_OIL_GAS using 'dplyr', 'ggplot2', and Other Tidy Tools 
...
# # 0) --- See prior steps in R/fonts.R
# library(showtext)
...
Since some pre-processing steps are computationally expensive, I did that in a separate step (`analysis/01a_WB_project_pdo_prep.qmd`), WHERE:  

1. Retrieved manually ALL WB projects (**22,569**) approved **between FY 1947 and 2026** as of *31/08/2024* using simply the `Excel button` on this page [WBG Projects](https://projects.worldbank.org/en/projects-operations/projects-list?str_fiscalyear=1979&end_fiscalyear=1979&os=0)
...
Lastly (and this was quite surprising to me) the **remaining, viable 8,811 unique projects**, were matched by **only 7,582 unique PDOs**!
In fact, **2,235 projects share 1,006 NON-UNIQUE PDO text** in the "cleaned" dataset. Why? Apparently, the same PDO is re-used for multiple projects (from 2 to as many as 9 times), likely in cases of follow-up phases of a *parent* project or components of the same lending program."
...
pdo_train_t <- pdo_train_t %>%
   ...
   mutate(tok_sector_broad = case_when(
      ...
      stringr::str_detect(token_l, regex("Gender|Women|Girl|Woman|femal|Gender Equal|gender-base|gender inclus|gender mainstream|gender sensit|gender respons|gender gap|gender-based|gender-sensitive|gender-responsive|gender-transform|gender-equit|gender-balance", ignore_case = T)) ~  "GENDER_EQUAL" ,
            stringr::str_detect(token_l, regex("Climate|Environment|Sustain|Resilience|Adaptation|Mitigation|Green|Eco|Eco-|carbon|carbon cycle|carbon dioxide|climate change|ecosystem|emission|energy effic|greenhouse|greenhouse gas|temperature anomalies|zero net|green growth|low carbon|climate resilient|climate smart|climate tech|climate variab", ignore_case = T)) ~ "CLIMATE" ,
      TRUE ~ NA_character_)) %>% 
...
wdr2_set <- wdr2 %>% 
  ...
  mutate(url_keys = kableExtra::cell_spec(url_keys, link = url_keys))
...
highlight_words <- c("mitigate", "adapt")
...
clch_close_k <- close_words_sort %>% 
   ...
   row_spec(which(close_words_sort$contains_what %in% highlight_words), 
            background = "#d8e600") # Light yellow background
...
1. Paradossalmente, per chi si cimenta con l’utilizzo di questi strumenti di IA (ChatGPT, ecc.), appare presto evidente che per quanto stupefacenti, le sue risposte rivelano-per contrasto-l'incredibile complessità dell'intelligenza umana e della nostra capacità di comprendere il mondo.

Per fare qualche esempio:
   + la capacita di collegare concetti e idee in modo creativo e originale
   + *heuristics*, ovvero quelle scorciatoie mentali che ci permettono di prendere decisioni rapide e intuitive basate su esperienza e contesto 
   + la capacità di comprendere il contesto e di adattare il nostro comportamento di conseguenza
   + i principi etici e morali che guidano le nostre azioni e decisioni
   + l'autocoscienza, ovvero la consapevolezza di noi stessi, dei nostri pensieri e azioni  
ma allora cos'e' che sta scatenando tutto questo interesse per l'IA e la sua capacità di generare testi e risposte?
...
## Trend PDO words v sector (the tag) 
...
MATCH PDO WORDS AND SECTOR TAGS
 ...
 pdo_train_t$tok_sector_broad      n  sector_broad_tag$sector1_broad 
...
**Correlation Coefficients**: Display the correlation between `n_pdo` and `n_tag` within each facet.
...
# Usage example for a specific sector
f_plot_sector_trends(data = sector_broad_combo, sector_name = "ENERGY",
                   title = "Comparing frequency of sector trends over time" ,
                subtitle = "\`n_pdo\` = frequency in PDOs text; \`n_tag\` = frequency sector label (normalized values) \nKolmogorov-Smirnov test for similarity between the two trends")
```
...
# Usage example for a specific sector
f_plot_sector_trends(data = sector_broad_combo, sector_name = "TRANSPORT",
                   title = "Comparing frequency of sector trends over time" ,
                subtitle = "\`n_pdo\` = frequency in PDOs text; \`n_tag\` = frequency sector label (normalized values) \nKolmogorov-Smirnov test for similarity between the two trends")
...
# Usage example for a specific sector
f_plot_sector_trends(data = sector_broad_combo, sector_name = "MINING_OIL_GAS",
                   title = "Comparing frequency of sector trends over time" ,
                subtitle = "\`n_pdo\` = frequency in PDOs text; \`n_tag\` = frequency sector label (normalized values) \nKolmogorov-Smirnov test for similarity between the two trends")
```
...
# Usage example for a specific sector
f_plot_sector_trends(data = sector_broad_combo, sector_name = "ICT",
                   title = "Comparing frequency of sector trends over time" ,
                subtitle = "\`n_pdo\` = frequency in PDOs text; \`n_tag\` = frequency sector label (normalized values) \nKolmogorov-Smirnov test for similarity between the two trends")
```
...
# Usage example for a specific sector
f_plot_sector_trends(data = sector_broad_combo, sector_name = "HEALTH",
                   title = "Comparing frequency of sector trends over time" ,
                subtitle = "\`n_pdo\` = frequency in PDOs text; \`n_tag\` = frequency sector label (normalized values) \nKolmogorov-Smirnov test for similarity between the two trends")
```
...
# Usage example for a specific sector
f_plot_sector_trends(data = sector_broad_combo, sector_name = "EDUCATION",
...
## Trend PDO words v sector ($$)

#### ----  sum of `curr_total_commitment` corresponding to each sector1_broad per year 

> THIS STARTs FROM `projs_train` bc I needed the PROJECTS  
 
I need a table in long form with the following columns: `FY_appr`, `sector1_broad`, `sum_curr_total_commitment`

```{r}
# prep data
proj_train_commit <- projs_train %>% 
...
This I can bind with the `sector_broad_combo` to get the `n_pdo` and `n_tag` for each sector1_broad per year. 
 
```{r}

# THE FIRST TABLE 
sector_broad_combo2 <- sector_broad_combo %>% 
   dplyr::filter(tok_sector_broad %in% c("WAT_SAN", "ENERGY","TRANSPORT", "MINING_OIL_GAS", "ICT", "HEALTH", "EDUCATION", "URBAN"))

tabyl(sector_broad_combo2$tok_sector_broad, show_missing_levels = FALSE)
                  #        WAT_SAN 23 0.15231788   x
                  #         ENERGY 23 0.15231788   x  
                  #      TRANSPORT 23 0.15231788   X 
                  #            ICT 21 0.13907285   ~
                  # MINING_OIL_GAS 15 0.09933775   ~
                  #         HEALTH 23 0.15231788   X
                  #     EDUCATION 23 0.15231788    X
tabyl(proj_train_commit$sector1_broad)
                       #        WAT_SAN 23 0.15231788    x 
                       #         ENERGY 23 0.15231788    x 
                       #      TRANSPORT 23 0.15231788    X 
                       #            ICT 21 0.13907285    ~ 
...
# bind
sector_broad_freq_v_commit <- left_join(sector_broad_combo, 
                                        proj_train_commit, by = c("FY_appr", 
                                                                  "tok_sector_broad" =  "sector1_broad")) %>% 
   mutate(sum_curr_total_commitment = ifelse(is.na(sum_curr_total_commitment), 0, sum_curr_total_commitment),
          sum_comm_mln = sum_curr_total_commitment/1000000) %>%
   select(-n_tag) %>% 
   rename(sector = tok_sector_broad )

paint(sector_broad_freq_v_commit)

# count the rows per sector1_broad per year
counts <- sector_broad_freq_v_commit %>% 
   group_by(FY_appr ) %>% 
   summarise(n = n()) %>% 
   ungroup() %>% 
   rename(n_pdo = n)
...
Using `relative frequencies` or `normalized values` aligns better with comparing distributions that have vastly different ranges, like your `n_pdo` (token count) and `sum_curr_total_commitment` (funding amount).
```{r}
# Calculate relative frequency for n_pdo and sum_curr_total_commitment
sector_broad_freq_v_commit <- sector_broad_freq_v_commit %>%
  group_by(FY_appr) %>%
  mutate(
    rel_freq_n_pdo = n_pdo / sum(n_pdo) * 100,  # Relative frequency for n_pdo in percentage
    rel_freq_commitment = sum_curr_total_commitment / sum(sum_curr_total_commitment) * 100  # Relative frequency for funding in percentage
  ) %>%
  ungroup() %>% 
   select(-sum_comm_mln)

# View the modified tibble
sector_broad_freq_v_commit
```


#### --- 🟢 [TAB] Chi-Square Test test of similarity with a table 

+ The `chi-square test` is used to evaluates whether two categorical distributions have the same proportions across categories.
+ The `null hypothesis` is that The relative frequencies of `n_pdo` and `sum_curr_total_commitment` are distributed *similarly* across categories (in this case, sectors). This means that any observed differences in category proportions are due to chance.
- A `p-value` < 0.05 indicates a significant difference, suggesting the distributions are *dissimilar.*
+ The Chi-Square Test does not require the data to follow a normal distribution because it operates on observed versus expected frequencies rather than on assumptions about the underlying data distribution.
+ the Chi-Square Test's reliability improves with larger counts. 


```{r}
#| label: tab-CHI_sec_comm
#| eval: true
#| echo: false
#| output: true

# Run the Chi-Square Test on the relative frequencies for each FY_appr
chi_square_test_by_sec_k <- sector_broad_freq_v_commit %>%
  group_by(sector) %>%
  summarise(
    chi_square_stat = chisq.test(
      x = rel_freq_n_pdo,
      y = rel_freq_commitment
    )$statistic,  
    chi_square_p_value = chisq.test(
      x = rel_freq_n_pdo,
      y = rel_freq_commitment#,
      # Use Monte Carlo simulation to estimate the p-value
      # simulate.p.value = TRUE,
      # B = 1000
    )$p.value
  ) %>% 
   mutate(similarity = if_else(chi_square_p_value < 0.05,  "Dissimilar", "Similar")) %>%
   arrange( desc(chi_square_p_value))

# # View the test results
# chi_square_test_by_sec_k

# save as object
write_rds(chi_square_test_by_sec_k, here("analysis", "output", "tables" ,"chi_square_test_by_sec_k.rds"))

# Count how often the phrase appears in the vicinity of the target bigram
chi_square_test_by_sec_k %>% 
   kable(format = "html", 
         col.names = c("SECTORS", "Chi2 statistic","Chi2 p-value",  "Distributions"),
         # Round  to 4 digits) 
         digits = c(0, 4, 4, 0)) %>% 
   kable_styling(full_width = FALSE) %>%
   row_spec(which(chi_square_test_by_sec_k$similarity == "Dissimilar"), background = "#e7d8da")
```
 
#### --- Calculate Cumulative Relative Frequencies
Calculate Cumulative Distributions: `cum_rel_freq_n_pdo` and `cum_rel_freq_commitment` represent the cumulative relative frequencies within each sector.

+ The K-S Test compares the cumulative distribution functions (CDFs) of two continuous distributions , yielding a statistic (ks_statistic) and p-value (ks_p_value).
+ The `null hypothesis` is that the cumulative distributions of `cum_rel_freq_n_pdo` and `cum_rel_freq_commitment` are identical. This implies that the two distributions have the same shape and cumulative behavior over the range of values.
```{r}
sector_broad_freq_v_commit <- sector_broad_freq_v_commit %>%
  group_by(sector) %>%
  arrange(FY_appr) %>%
  mutate(
    cum_rel_freq_n_pdo = cumsum(rel_freq_n_pdo) / sum(rel_freq_n_pdo),  # Cumulative distribution for n_pdo
    cum_rel_freq_commitment = cumsum(rel_freq_commitment) / sum(rel_freq_commitment)  # Cumulative distribution for commitment
  ) %>%
  ungroup()

```

#### --- [TAB] K-S Test for Each Sector
```{r}
#| label: tab-KS_sec_comm
#| eval: true
#| echo: false
#| output: true

ks_test_by_sector <- sector_broad_freq_v_commit %>%
  group_by(sector) %>%
  summarise(
    ks_statistic = ks.test(
      x = cum_rel_freq_n_pdo,
      y = cum_rel_freq_commitment
    )$statistic,
    ks_p_value = ks.test(
      x = cum_rel_freq_n_pdo,
      y = cum_rel_freq_commitment
  ...
  mutate(similarity = if_else(ks_p_value < 0.05, "Dissimilar", "Similar")) %>%
  arrange(ks_p_value)
...
# save as object
write_rds(ks_test_by_sector, here("analysis", "output", "tables" ,"ks_test_by_sector.rds"))

# Count how often the phrase appears in the vicinity of the target bigram
ks_test_by_sector %>% 
   kable(format = "html", 
         col.names = c("SECTORS", "K-S statistic","K-S p-value",  "Distributions"),
         # Round  to 4 digits) 
         digits = c(0, 4, 4, 0)) %>% 
   kable_styling(full_width = FALSE) %>%
   row_spec(which(ks_test_by_sector$similarity == "Dissimilar"), background = "#e7d8da")

```

KEY DIFF

> The Chi-Square Test focuses on differences in proportions within categories, making it suitable for frequency comparisons across discrete categories.

> The K-S Test compares the cumulative distribution functions (CDFs) of two continuous distributions,  making it suitable for comparing the shape and cumulative trends of continuous distributions.
 
### [FUNC] Plot each rel_freq_commitment v rel_freq_n_pdo BY sector 
```{r}
# --- Split data long into a LIST of subset by sector
commit_by_sector_list <- base::split(sector_broad_freq_v_commit, sector_broad_freq_v_commit$sector) 
str(commit_by_sector_list) 
...
f_plot_comm_sector( name_sec = "ENERGY")
f_plot_comm_sector( name_sec = "TRANSPORT")
f_plot_comm_sector( name_sec = "ICT")
f_plot_comm_sector( name_sec = "HEALTH")
f_plot_comm_sector( name_sec = "EDUCATION")
f_plot_comm_sector( name_sec = "MINING_OIL_GAS")
 
# Use purrr::map to apply the function over the names of sector_list
 map(names(commit_by_sector_list), f_plot_comm_sector) 

``` 
 
#### --- [FIG] ICT  

Trend of PDO words v sector (the \$$ )

According to Chi Square Test, all sector are similar, but the distributions in the ICT sector are THE *least similar* (p = 0.2306 is the smallest). 
...
According to Chi Square Test, all sector are similar, but the distributions in the "MINING_OIL_GAS"  sector are THE *most similar* (p = 0.2665 is the biggest). 
```{r}
#| label: fig-comp_commit_min
#| eval: true
#| echo: false
#| output: true
#| fig.width: 10 # adjust based on text width in inches
