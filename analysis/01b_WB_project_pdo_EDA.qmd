...
+ check if the peaks in sector words correspond to peak in sector (the tag) 
+ check if the peaks in sector words correspond to  sector (the  \$$ )
...
close_words <- sentences_with_climchang %>%
  mutate(contains_what = case_when(
     str_detect(sentence, "mitigat") ~ "mitigate",
     str_detect(sentence, "adapt") ~ "adapt",
     str_detect(sentence, "vulnerab") ~ "vulnerability",
     str_detect(sentence, "hazard") ~ "hazard",
     str_detect(sentence, "resil") ~ "resilience",
     TRUE ~ "..."))
...
sentences_with_climchang2_k <-  sentences_with_climchang %>%
   ...
   mutate(contains_what = case_when(
      str_detect(closest_text, "mitig") ~ "mitig",
      str_detect(closest_text, "adapt") ~ "adapt",
      str_detect(closest_text, "vulnerab") ~ "vulnerab",
      str_detect(closest_text, "hazard") ~ "hazard",
      str_detect(closest_text, "resil") ~ "resil",
      TRUE ~ "...")) %>% 
   filter(contains_what != "...") %>% 
