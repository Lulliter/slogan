...
knitr::include_graphics("img/phi.png")
```
...
word_cors %>%
  filter(item1 %in% c( "institutions", "people", "governments", "markets")) %>%
  group_by(item1) %>%
  top_n(6) %>%
  ungroup() %>%
...
##### --> (EXE) Visualizing Collocations
We will now use an example of one individual word ( _development_ ) to show, how collocation strength for individual terms is calculated and how it can be visualized. 
