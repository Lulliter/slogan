...
In this section, I recount some attempts to enhance the quality of the metadata by predicting missing features in the World Bank project documents. The idea is to use the text of the Project Development Objective (PDO) section as input data to predict the missing *tags* (`sector`, `environmental risk category`, etc.) for some of the observations.
...
To predict a missing feature (e.g., `sector`) based on text data and other available predictors, several supervised machine learning (ML) algorithms can be applied. I tested a few different algorithms suitable when you have a mixture of text and structured data. 
...
::: {.callout-warning icon="false" collapse="true" style="background-color: #fffcf9;"}
### {{< bi terminal-fill color=#9b6723 >}} Nerdy Note
### Steps of prediction in ML

1.  `outcome label engineering` Define what we want to predict (outcome variable, $y$), and its functional form (binary or multiclass, log form or not if numeric)
