...
In this learning project, I explore a dataset of World Bank Projects & Operations, with a focus on the text data contained in the **Project Development Objective (PDO) section** of the Bank's projects (loans, grants, technical assistance). The **PDO** outlines, in synthetic form, the proposed objectives of operations, as defined in the early stages of the World Bank project cycle. <!-- This includes: 1) Identification, 2) Preparation, 3) Appraisal, 4) Negotiation/approval, 5) Implementation, and 6) Completion/validation and evaluation.  -->
...
The original dataset contained **22,569** WB projects approved **between fiscal years 1947 and 2025** as of *August 31, 2024*. Of these, approximately 50% (**11,322 projects**) had a *viable* **PDO** text in the dataset (not blank or labeled as "*TBD*", etc.), all approved after FY2001. From this subgroup, some projects were dropped: **3 projects** lacking *project status*, **2,176 projects** without *"board approval FY"*, and **332 projects** with approval still pending as of September 2024.

As a result, the **usable** data for the analysis include **8,811 projects**.

Surprisingly, within this *clean* subsample, **2,235 unique projects share only 1,006 unique PDOs among them**: *recycled PDOs* seem to appear in cases of follow-up projects or components of a parent project.
...
# Most frequent combination of words in the PDO text data
...
# Sectors in the PDO text data
To narrow down the exploration of `tokens`, I looked into the frequency of specific **sector terms** in the PDO text data. To better capture the concept of "sector", I created a **broad SECTOR variable** to encompass all relevant words in a broader definition. 

::: {.callout-note collapse="true" style="background-color: #e7edfa;"}
### *Custom* Sector Definitions
The ***"sector"*** term discussed here is *not* the `sector` variable available in the data, but it is an artificial construct reflecting the occurrence of a group of terms referred to a sector topic. It is based on a logical association of certain words with a sector definition, rooted in the World Bank's own classification of sector and sub-sector.

Here is the approach used to create a *"broad SECTOR" variable* to group the sectors into broader categories:
...
A possible explanation is that the PDOs may may echo themes from the **World Development Reports (WDR)**, the World Bank’s flagship annual publication that analyzes a key development issue each year. Far from being speculative research, each WDR is grounded in the Bank’s field-based insights and, in turn, it informs the Bank's policy and operational priorities. This would suggest a likely alignment between WDR themes and project objectives in the PDOs.

To some extent, visual exploration (see examples below) seems to support this hypothesis: thematically relevant WDRs consistently *appear* in close proximity to peaks in sector-related term frequencies. However, further validation is necessary. Additionally, preparing each WDR typically takes 2-3 years, so a temporal alignment with project documents may naturally include some lag.
...
## Examples of sector-related term frequency within the PDO text
...
# Comparing PDO text against *metadata* elements (`sector`)

The available data includes not only text but also relevant metadata, such as the `sector1` variable, which indicates the project’s primary sector. Do the terms in the PDO text align with this sector label? To examine this, I applied the **two-sample Kolmogorov-Smirnov test** to compare the distribution of `sector-related terms` in the PDO text with the distribution of `sector1`.
...
# Comparing PDO text against *metadata* elements (`amount committed`)

In line with the previous point, is it just words or do trends observed in PDOs' text actually reflect the allocation of funds to specific sectors? To explore this question, we follow the same approach as before, but this time we compare the distribution of sector-related terms in the PDOs' text with the distribution of variable `amount committed`, provided in the project data.

let us pick a couple of examples in which a sector/theme is also declared by dedicated variables. In these cases, the dataset enables to identify projects focused on:

+ **EDUCATION**, which is codified as the first *sector* in the metadata accompanying the PDOs.  
+ **CLIMATE CHANGE**, which is codified as as the first *theme* in the metadata accompanying the PDOs. 


### 🔶 Education sector: words v. funding 


### 🔶 Climate change theme: words v. funding 
