...
To predict the missing binary categorical outcome `env_cat_f2`, I tried several models, including: ***Lasso losgistic regression*** (with different specifications including only text or a mix of text and other predictors) and ***Naive Bayes classification*** (Here I only report the results, but details can be found on this  [webpage](https://lulliter.github.io/slogan/analysis/01c_WB_project_pdo_feat_class.html)). 
...
-   With all the buzz around AI and Large Language Models (LLMs), this kind of analysis might seem yesterday news. But I think there’s still huge, untapped potential for using NLP in development studies, policy analysis, and beyond—especially when it’s backed by domain expertise.

# Acknowledgements

Below are some great resources--geared especially toward [{{< fa brands r-project >}}]{style="color: #3b7697;"} programmers-- to learn and implement NLP techniques (👏🏻👏🏻). 
