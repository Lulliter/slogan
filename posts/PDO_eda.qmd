...
These results are not impressive by any means, but, once again, my main goal was to explore possible operations that can be done with the data such as the one in this case study, so minimal effort was put into optimizing the model performance (by way of preprocessing, features engineering, algorithm choice, different tuning of the hyperparameters, etc.).
