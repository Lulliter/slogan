...
description: |
  ...
  This is an ongoing project, so comments, questions, and suggestions are welcome. The R source code is open (still in progress, not very polished).<br>
...
+ **KS statistic**: The vectors of observed distributions have been normalized (bringing `n_pdo` and `n_tag` to a [0, 1] range before applying the Kolmogorov-Smirnov (KS) test). This is useful when distributions differ substantially in scale or units, as it makes them directly comparable in relative terms.
:::
...
For example, looking at most recurrent `bigrams` (two-word combinations) in the PDO text, the phrase *"eligible crisis"* caught my attention. It appears in the PDOs of 112 projects, and in 32% of these cases, it is accompanied by either *"respond promptly and effectively"* or *"immediate and effective response"*. @tbl-elgcris shows a few examples, suggesting this might be a recurring standard phrasing.
...
In this section, I recount some attempts to enhance the metadata quality by predicting missing features in the World Bank project documents. The idea is to use the text of the Project Development Objective (PDO) section as input data to predict the missing *categorical descriptors*, etc.) for some of the observations (`sector`, `environmental risk category`. @tbl-missing_ML shows some missing features in the source dataset.
...
#| echo: false
#| output: true
#| tbl-cap: Missing feature in source dataset
...
The goal at hand has to do with ***text classification***, that is assigning categories to some observations. To predict a missing feature based on text data and other available predictors, several machine learning (ML) algorithms can be applied. I tested a few different algorithms suitable when you have a mixture of text and structured data. 
...
3) **Testing 2 set** (with `env_cat` missing) 1,167 observations

![Sampling strategy](../images/Sample_EnvCat2.png){fig-align="center"}

## Choosing the ML algorithm

To predict the missing binary categorical outcome `env_cat_f2`, I tried several algorithms, including: ***Lasso losgistic regression*** (with different specifications including only text or a mix of text and other predictors) and ***Naive Bayes classification*** (Here I only report the results, but details can be found on this  [webpage](https://lulliter.github.io/slogan/analysis/01c_WB_project_pdo_feat_class.html)). 
...
#### The steps to predict the missing feature were as follows: 
...
The best model performance was achieved by the LASSO logistic regression model that combined both PDOs' text and some available metadata infomration to predict the missing `env_cat_f2` in the testing set. The model achieved an accuracy of 0.79 and an ROC-AUC of 0.85.:


## Evaluating the performance of the preferred ML model to predict a missing feature
@fig-ML_final_fit_cm reports the `confusion matrix` of the preferred ML model to predict the missing `environment risk category` assigned to the World Bank projects, as it displays the breakdown of true and predicted classifications. A strong performing model would show the bulk of the observations (or a darker shade) top-left quadrants (`true positives`), and the  bottom-right quadrants (`true negatives`). In this case, the model seems to be doing a good job in predicting the `environment risk category` in the **High-Med** group, while it struggles with the **Low & Other** group. In fact many of them are wrongly classified as `High-Med Risk` (`false positives`).  
...
It is very interesting also to explore which coefficient values are the most important in the model. This can be done by looking at the `feature importance` plot ( @fig-ML_feature_importance).
...
These results are not impressive by any means; however, the main objective here was to explore potential analysis that can be conducted with the data at hand. With additional effort put into preprocessing, feature engineering, refined algorithm selection, and hyperparameter tuning, there is significant potential to enhance model performance further.  

While I don't relate it here, I explored also the prediction of a multiclass outcome (`sector`, combined into 7 levels). However, the results were not as good as the binary classification. This is not surprising given that the task become much more complex, especially with imbalanced data or limited sample size. 
 
  
<!-- ## Some considerations on the predicting process -->
<!-- While I didn't spend much time in increasing the predictive performance, I want to mention some steps that could be taken to improve the model performance:  -->
...
<!-- to predict the missing `sector` tag in the World Bank project documents.   -->

<!--  Given that you have a mixture of text and structured data, here are some suitable algorithms: -->
...
-   Next steps could include delving deeper into hypothetical explanations for the patterns observed, e.g. by combining NLP on this document corpus with other data sources (e.g. information on other WB official documents and policy statements). Also further exploring NLP techniques for topic modeling, sentiment analysis, and other text-driven insights.
...
Below are some great resources to learn and implement NLP techniques--geared toward R programmers.
