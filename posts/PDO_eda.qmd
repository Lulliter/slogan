...
## Bigrams 
@fig-bigr shows the most frequent bigrams in the PDO text data. Results are not surprising in terms of frequent bigram recurrence:
...
+ A bit mysterious is perhaps *"eligible crisis"* (> 100 mentions)?!
...
Seeking some meaningful *tokens*, I looked into the frequency of specific **sector terms** in the PDO text data. To better capture the concept of "sector", I created a **broad SECTOR variable** to encompass all relevant words in a broader definition. 
...
A possible explanation is that the PDOs may reflect themes addressed in the **World Development Reports (WDR)**, the World Bank’s annual flagship publication that provides an in-depth analysis of a key development issue. Far from being speculative research, each WDR is grounded in the Bank’s concrete, field-based insights from projects and operations. These reports, in turn, inform the Bank's policy priorities and operational strategies.  
Thus, it is reasonable to expect a degree of alignment between the themes discussed in the WDRs and the projects' objectives outlined in the PDOs.

To some extent, visual exploration (examples provided below) seems to support this hypothesis: thematically relevant WDRs consistently *appear* in close proximity to peaks in sector-related term frequencies. However, further validation is necessary. Additionally, it’s important to note that:

+ Sector terms are not always captured with the same *precision*, which affects the association.
+ Each WDR is the culmination of an extensive empirical research process, typically spanning 2-3 years. Therefore, a temporal alignment with project documents may naturally include some lag.

## Examples of peaks in sector-related term frequency within the PDO text data

@fig-agr shows a "combined sector" that is quite broadly defined (**AGRICULTURE, FORESTRY, FISHING**) with the highest peak in 2010, two years after the publication of the WDR on **"Agriculture for Development"**.
...
# Comparing PDO text data with *metadata* elements (`sector`)

Do information contained in the PDOs' text actually reflect the sector assignment in the metadata? To explore this question, I used the **two sample Kolmogorov-Smirnov test** to compare the distribution of sector-related terms in the PDOs' text with the distribution of variable `sector1`, provided in the project data.  
In @tbl-comp_sec, the results show that the distributions are similar in most of the sectors observed. This is quite good news, because it means that (in the absence of proper metadata availability) the PDOs' text data can be used to infer the sector assignment with a reasonable degree of accuracy. 
...
@fig-comp_transp shows the distributions of the **TRANSPORT** sector in the  PDOs' text data and the metadata. The two distributions are the most similar, as confirmed by the Kolmogorov-Smirnov test with a p-value of 0.641. 
```{r}
...
# Comparing PDO text data with *metadata* elements (`amount committed`)
